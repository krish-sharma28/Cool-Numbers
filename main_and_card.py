...
class Creation:
    ...
    def __str__ (self):
        """ Returns a better looking version of the card.
            Written by Jordan Bennett

        """
        card_hand = self.draw_unique_hand()
        rep = f"""
-------------------
| {card_hand[0]} | {card_hand[1]} | {card_hand[2]} | {card_hand[3]} |
-------------------
        """
        return rep
...
def play_game(players, rounds):
    ...
    for r in range(1, rounds + 1):
        ...
        if validate_expression(solution, list(game.current_hand)): # needs edit
